...
def draw_detections(img: np.array, bboxes: List[List[int]], classes: List[int], names: List[str], conf: List[float], colors):
    line_thickness = 1
    twenty_five_percent_width = int(img.shape[1] * 0.25)
    r1, r2 = (int(img.shape[1]/2)-twenty_five_percent_width,0),(int(img.shape[1]/2)+twenty_five_percent_width,img.shape[0])
    tl = line_thickness or round(
        0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    tf = max(tl - 1, 1)  # font thickness
    img = cv2.rectangle(img, r1, r2, (0, 0, 255), thickness=tl, lineType=cv2.LINE_AA) 
