...
class YoloV7_ROS:
    ...
    def __img_cb(self, img_msg):
        ...
        # Assuming obj[4] represents the class ID
        person_class_mask = trackers[:, 4] == 0.0  # Assuming 0.0 represents the "person" class
        person_track_ids = trackers[person_class_mask, 8]  # Assuming track ID is in the 9th column (0-indexed)
        
        # Append person track IDs to self.current_person_ids
        self.current_person_ids = person_track_ids

        # Remove IDs of person class objects that are no longer detected
        removed_person_ids = np.array([id for id in self.previous_person_ids if id not in self.current_person_ids])
        #print("removed_person_ids",removed_person_ids)
        #print("curent_track_ids",person_track_ids)
        for removed_id in removed_person_ids:
            print("Person with track ID", removed_id, "is no longer detected")
            self.previous_person_ids = np.delete(self.previous_person_ids, np.where(self.previous_person_ids == removed_id))

        # Find new person track IDs
        new_person_ids = np.setdiff1d(self.current_person_ids, self.previous_person_ids)
        
        # Notify when new person track IDs are detected
        for track_id in new_person_ids:
            print("New person detected with track ID:", track_id)             
       
        tracks = self.tracker.getTrackers()
        # Update previous_person_ids with current_person_ids
        self.previous_person_ids = self.current_person_ids.copy()
